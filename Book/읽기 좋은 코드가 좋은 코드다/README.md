...
- 테스트 실패시 왜 틀렸는지 알 수 있는 메시지를 명확하게 표시해주는 게 좋다. 
- 테스트 입력 값은 테스트를 꼼꼼하게 확인해볼수 있는 값으로 설정해야하며 명확한 값을 설정하는게 좋다. 
- 그러나 하나의 완벽한 입력값을 사용하기보다 작은 테스트를 여러 개 사용하는 방식이 더 쉽고 효과적이다. 
- 테스트 메서드 이름으로 테스트를 상세하게 묘사할 수 있으면 더욱 좋다. 
- 테스트 하기 좋은 코드를 작성해야 테스트 작성이 쉬워진다. 
- 테스트 하기 좋은 코드의 특징이란? 
	- 클래스들이 내부 상태를 거의 가지고 있지 않아서 테스트하기전 설정할 일이 거의 없다. 
	- 클래스나 함수가 한번에 하나의 일만 수행한다.
	- 클래스가 독립적이고 의존적이지 않아서 각 클래스를 독립적으로 테스트할 수 있다.
	- 함수가 간단하고 잘 정의된 인터페이스를 가지고 있다. 
- 지나친 테스트는 경계하자. 꼭 필요할때만 테스트 코드를 작성하자. 프로토타입에 테스트 코드를 작성할 필요는 없겠지?