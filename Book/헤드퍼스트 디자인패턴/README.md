...
### 프로토타입 패턴
- 클라이언트는 새로운 인스턴스를 만드는 과정을 몰라도 객체를 생성할 수 있다.
- 인스턴스 생성 과정을 캡슐화한다.
- 기존 인스턴스를 복사해서 새로운 인스턴스를 생성한다. 

### 비지터 패턴
- 다양한 객체에 새로운 기능을 추가할 때 캡슐화가 중요하지 않은 상황에서 사용할 수 있다.
- 구조를 변경하지 않으면서 복합 객체 구조에 새로운 기능을 추가할 수 있다.
- 트래버서 객체는 컴포지트 패턴을 쓸 때, 복합 객체 내에 속해있는 모든 객체에 접근하는 일을 도와주는 역할을 한다.
- 비지터 객체는 복합 객체 내의 모든 객체를 대상으로 원하는 작업을 처리해주는 역할을 한다.
- 복합 객체 구조의 캡슐화가 깨지는 단점도 존재한다.